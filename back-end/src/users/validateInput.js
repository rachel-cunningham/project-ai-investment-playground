...
function validateField(value, type, criteria) {
    switch (type) {
        case "string":
            return (
                typeof value === "string" && value.length <= criteria.max
            )
        case "boolean":
    ...
    for (const field in validationRules) {
        ...
        if (value !== null && value !== undefined) {
            if (
                ...
                return res.status(400).json({ error: `Invalid ${field}, ${field} must be a ${validationRules[field].type} with a maximum length of ${validationRules[field].max}. Recieved: ${value}` })
            }
        }
    }
